...
import { JwtPanelPayload } from '../analyzers/jwt/panelPayload';
...
export class JwtClaimsetViewerPanel {
   ...
   * @param extensionUri The URI of the directory containing the extension.
   */
  public static render(extensionUri: Uri, payload: JwtPanelPayload) {
    if (!payload) {
      return;
    }
    ...
    JwtClaimsetViewerPanel._payload = payload;
  }
        ...
        switch (message.command) {
          case 'onDidInitialize': {
            JwtClaimsetViewerPanel.currentPanel?._panel.webview.postMessage(
              JwtClaimsetViewerPanel._payload
            );
            return;
          }
          case 'alert': {
